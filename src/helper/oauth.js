...
server.grant(oauth2orize.grant.code(function (client, redirectURI, user, ares, done) {
  var code = uuid.v4();
  db.authorizationCode.save(code, client.id, redirectURI, user.id, client.scope, function (err) {
    if (err) {
      return done(err);
    }
...
server.grant(oauth2orize.grant.token(function (client, user, ares, done) {
  var token = uuid.v4();
  db.accessToken.save(token, config.token.calculateExpirationDate(), user.id, client.id, client.scope, function (err) {
    if (err) {
      return done(err);
    }
